...
def ensure_vector_database() -> None:
    settings = get_settings()
    if settings.uses_pinecone or settings.uses_shared_database:
        return
...
def wait_for_databases(timeout_s: float = 90.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if app_ok and settings.uses_pinecone:
            return
        if app_ok:
            try:
                ...
                time.sleep(1.5)
                continue
        vec_ok = settings.uses_pinecone or _ready(settings.vector_dsn)
        if app_ok and vec_ok:
            return
        missing = []
        if not app_ok:
            missing.append("postgres")
        if not settings.uses_pinecone and not vec_ok:
            missing.append("pgvector")
        last_error = "waiting for " + ", ".join(missing)
        time.sleep(1.5)
    hint = (
        "Set DATABASE_URL for Postgres catalog, or start local databases with `docker compose up -d`."
        if settings.uses_pinecone or settings.uses_shared_database
